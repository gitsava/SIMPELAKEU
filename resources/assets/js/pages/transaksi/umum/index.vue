...
<script>
    import Tables from '~/pages/transaksi/umum/table'
    ...
    export default {
        ...
        created(){
            ...
            Cookies.set('t', 0, { expires: null })
        },
        methods: {
            getTransaksiUmum(idKategori,tahun){
                console.log(idKategori + ' ' + tahun)
                this.isloading = true;
                let url = '/api/transaksiumum/fetch?tahun='+tahun+'&idKategori='+idKategori
                console.log(url)
                fetch(url)
                  .then(res => res.json())
                  .then(res => {
            ...
            submitEdit(){
                ...
                this.form.patch(url)
                    .then(({data})=>{
                        this.getTransaksiUmum(this.selectedKategori,date.getFullYear())
